...
#
	...
	#traverse(func(room_config_data:Dictionary, location:Dictionary) -> void:
		#var room_base_states:Dictionary = base_states.room[U.location_to_designation(location)]
		#if !room_config_data.room_data.is_empty() and room_base_states.is_activated:
